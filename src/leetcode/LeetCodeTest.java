...
public class LeetCodeTest {
	 ...
	 * 任务调度器
	 *
	 * 假设数组 ["A","A","A","B","B","C"]，n = 2，A的频率最高，记为count =
	 * 3，所以两个A之间必须间隔2个任务，才能满足题意并且是最短时间（两个A的间隔大于2的总时间必然不是最短） ，因此执行顺序为：
	 * A->X->X->A->X->X->A，这里的X表示除了A以外其他字母，或者是待命，不用关心具体是什么，反正用来填充两个A的间隔的。
	 * 上面执行顺序的规律是： 有count - 1个A，其中每个A需要搭配n个X，再加上最后一个A，所以总时间为 (count - 1) * (n + 1) +
	 * 1 要注意可能会出现多个频率相同且都是最高的任务，比如
	...
	/**
	 * int[] inorder = new int[] {9,3,15,20,7}; int[] postorder = new int[]
	 * {9,15,7,20,3}; 根据一棵树的中序遍历与后序遍历构造二叉树。
	 * 
	 * @author zs
	 * @date 2019年4月29日
	 */
	 ...
	 * https://github.com/CyC2018/CS-Notes/raw/master/notes/pics/31d9adce-2af8-4754-8386-0aabb4e500b0.png
	 */
	...
	private static TreeNode reConstructBinaryTree(int[] pre, int preStart, int preEnd, int[] in, int inStart, int inEnd) {
		...
		for (int i = inStart; i < inEnd; i++) {
			if(in[i] == pre[preStart]) {
				root.left = reConstructBinaryTree(pre, preStart+1, i-inStart+preStart+1, in, inStart, i);
